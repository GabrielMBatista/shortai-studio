...
const GoogleLoginButton: React.FC<{ disabled: boolean }> = ({ disabled }) => {
  ...
  if (!csrfToken) return <Loader text={t('auth.preparing_login')} />;

  return (
    <form action={`${apiUrl}/auth/signin/google`} method="POST">
      <input type="hidden" name="csrfToken" value={csrfToken} />
      <input type="hidden" name="callbackUrl" value={callbackUrl} />
      <button type="submit" disabled={disabled} className="w-full bg-white hover:bg-slate-50 disabled:opacity-50 disabled:cursor-wait text-slate-900 font-semibold py-3.5 px-4 rounded-xl flex items-center justify-center transition-all transform hover:scale-[1.02] shadow-lg">
      ...
      <div className="max-w-md w-full bg-slate-800/50 backdrop-blur-xl border border-slate-700 p-8 rounded-2xl shadow-2xl z-10 relative">
        ...
        {isLoading && (
          <div className="absolute inset-0 bg-slate-900/80 backdrop-blur-sm z-50 rounded-2xl flex items-center justify-center">
            <Loader text={t('auth.authenticating')} />
          </div>
        )}
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-white mb-2">{t('auth.welcome_title')}</h1>
          <p className="text-slate-400">{t('auth.welcome_subtitle')}</p>
        </div>
        ...
        <div className="space-y-4 animate-fade-in-up">
          ...
          <p className="text-xs text-center text-slate-500 mt-4">
            {IS_DEMO_LOGIN ? t('auth.backend_connected') : t('auth.secure_login')}
          </p>
        </div>
      </div>
    </div>
