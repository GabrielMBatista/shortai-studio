...
const VideoPlayer: React.FC<VideoPlayerProps> = ({ scenes, onClose, bgMusicUrl, title = "shorts-ai-video" }) => {
  ...
  useEffect(() => {
    if (activeScene) {
      setRealDuration(activeScene.durationSeconds);
    }
  }, [activeScene]);

  useEffect(() => {
    if (activeScene) durationRef.current = realDuration * 1000;
  }, [realDuration, activeScene]);
    ...
    const handleAudio = async () => {
      try {
        if (isPlaying && hasAudio) {
          ...
          // Use realDuration for sync calculation
          const targetTime = realDuration > 0 ? (progress / 100) * realDuration : 0;

          // Only sync if drift is significant (> 0.5s) AND audio is loaded
          if (!audioRef.current!.paused && Math.abs(audioRef.current!.currentTime - targetTime) > 0.5) {
            // Check if audio metadata is loaded before seeking
  ...
  }, [currentSceneIndex, isPlaying, activeScene, bgMusicUrl, realDuration]); // Added realDuration dependency
  ...
  // Use realDuration for subtitle sync
  const currentTimeInScene = (progress / 100) * realDuration;
    ...
    <div className="fixed inset-0 z-50 bg-black/95 flex flex-col items-center justify-center p-4 backdrop-blur-3xl">
      <audio
        ...
        onLoadedMetadata={(e) => {
          ...
          if (d && isFinite(d) && d > 0) {
            // Update duration if audio is longer than estimated or significantly different
            // We trust audio duration as the source of truth for the scene length
            setRealDuration(d);
          }
        }}
      />
      ...
      <div className="relative w-full max-w-[420px] aspect-[9/16] bg-black rounded-[2.5rem] shadow-2xl overflow-hidden ring-4 ring-slate-800/50">
        <div className="absolute inset-0 overflow-hidden">
          <img key={activeScene.sceneNumber} src={activeScene.imageUrl} className={`w-full h-full object-cover transition-opacity duration-500 ${isPlaying ? 'animate-ken-burns' : ''}`} style={{ animationDuration: `${realDuration + 2}s` }} />
